---
title: "map-3: DBA fields with finite fringes"
...
intervals. Its optional OPALX run uses one rank, one thread, `MAXSTEPS=1` and
`--info 2`, saving `map-3-ring.out`, geometry and provenance. The default
...
The single-rank `dop853-validation` run completed with those settings and printed
...
these fringe fields and larger body separation. Subsequent studies use one MPI
rank and save stdout with `--info 2`.
...
per map integrator was run sequentially, with one MPI rank and one OpenMP thread.
...
single-rank map-2 observations. The previous campaign was incomplete, so these
