...
| `LINEARTRANSFERMAPINTEGRATOR` | `BORIS` | Numerical stepping for both the map-enabled reference orbit and perturbed rays. `LF2` is an alias for the same Boris pusher. Other methods are rejected, not silently substituted. |
...
boundary resolution and subtraction noise remain. The ray integrator still has second-order
accuracy in smooth fields; Richardson here changes differentiation order, not time-integration
order. These controls do not select the production-particle integrator.
